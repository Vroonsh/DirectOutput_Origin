...
namespace DirectOutput.LedControl.Setup
{
    ...
    public class Configurator
    {
        ...
        public void CreateEffect(TableConfigColumn TCC, int SettingNumber, Table.Table Table, IToy Toy, int LedWizNr, string IniFilePath, string ShortRomName)
        {
            TableConfigSetting TCS = TCC[SettingNumber];
            ...
            if (Toy is IMatrixToy<RGBAColor> || Toy is IMatrixToy<AnalogAlpha>) {
                ...
                if (!TCS.ShapeName.IsNullOrWhiteSpace()) {
                    if (Toy is IMatrixToy<RGBAColor>) {
                        ...
                        if (ActiveColor != null) {
                            ...
                            Effect = new RGBAMatrixColorScaleShapeEffect() { ActiveColor = ActiveColor, InactiveColor = InactiveColor, LayerNr = Layer, FixedLayerNr = TCS.Layer.HasValue, ShapeName = TCS.ShapeName, Height = TCS.AreaHeight, Width = TCS.AreaWidth, Top = TCS.AreaTop, Left = TCS.AreaLeft, ToyName = Toy.Name };
                        } else {
                            //No color defined. Use org color effects
                            Effect = new RGBAMatrixShapeEffect() { LayerNr = Layer, FixedLayerNr = TCS.Layer.HasValue, ShapeName = TCS.ShapeName, Height = TCS.AreaHeight, Width = TCS.AreaWidth, Top = TCS.AreaTop, Left = TCS.AreaLeft, ToyName = Toy.Name };
                    ...
                    if (Toy is IMatrixToy<RGBAColor>) {
                        ...
                        Effect = new RGBAMatrixPlasmaEffect() { ActiveColor = ActiveColor1, ActiveColor2 = ActiveColor2, InactiveColor = InactiveColor, Height = TCS.AreaHeight, Width = TCS.AreaWidth, Top = TCS.AreaTop, Left = TCS.AreaLeft, LayerNr = Layer, FixedLayerNr = TCS.Layer.HasValue,  PlasmaSpeed = TCS.PlasmaSpeed, PlasmaDensity = TCS.PlasmaDensity, ToyName = Toy.Name };
                    } else {
                    ...
                    if (Toy is IMatrixToy<RGBAColor>) {
                        ...
                        if (ActiveColor != null) {
                            ...
                            if (TCS.AreaDirection != MatrixShiftDirectionEnum.Invalid) {
                                ...
                                //Color effect
                                Effect = new RGBAMatrixColorEffect() { ActiveColor = ActiveColor, InactiveColor = InactiveColor, Height = TCS.AreaHeight, Width = TCS.AreaWidth, Top = TCS.AreaTop, Left = TCS.AreaLeft, LayerNr = Layer, FixedLayerNr = TCS.Layer.HasValue,  ToyName = Toy.Name };
                            }
                        ...
                        if (TCS.AreaDirection != MatrixShiftDirectionEnum.Invalid) {
                            //shift effect
                            Effect = new AnalogAlphaMatrixShiftEffect() { ShiftDirection = TCS.AreaDirection, ShiftAcceleration = TCS.AreaAcceleration, ActiveValue = ActiveValue, InactiveValue = InactiveValue, Height = TCS.AreaHeight, Width = TCS.AreaWidth, Top = TCS.AreaTop, Left = TCS.AreaLeft, LayerNr = Layer, FixedLayerNr = TCS.Layer.HasValue,  ToyName = Toy.Name };
                            if (TCS.AreaSpeed > 0) {
                            ...
                            //flicker effect
                            Effect = new AnalogAlphaMatrixFlickerEffect() { Density = TCS.AreaFlickerDensity.Limit(1, 99), ActiveValue = ActiveValue, InactiveValue = InactiveValue, Height = TCS.AreaHeight, Width = TCS.AreaWidth, Top = TCS.AreaTop, Left = TCS.AreaLeft, LayerNr = Layer, FixedLayerNr = TCS.Layer.HasValue,  ToyName = Toy.Name };
                            if (TCS.AreaFlickerMinDurationMs > 0) {
                            ...
                            //Color effect
                            Effect = new AnalogAlphaMatrixValueEffect() { ActiveValue = ActiveValue, InactiveValue = InactiveValue, Height = TCS.AreaHeight, Width = TCS.AreaWidth, Top = TCS.AreaTop, Left = TCS.AreaLeft, LayerNr = Layer, FixedLayerNr = TCS.Layer.HasValue,  ToyName = Toy.Name };
                        }
                ...
                if (ActiveColor != null) {
                    RGBAColor InactiveColor = ActiveColor.Clone();
                    InactiveColor.Alpha = 0;
                    Effect = new RGBAColorEffect() { ToyName = Toy.Name, LayerNr = Layer, FixedLayerNr = TCS.Layer.HasValue,  ActiveColor = ActiveColor, InactiveColor = InactiveColor };
                }
                ...
                AnalogAlpha InactiveValue = ActiveValue.Clone();
                InactiveValue.Alpha = 0;
                Effect = new AnalogToyValueEffect() { ToyName = Toy.Name, LayerNr = Layer, FixedLayerNr = TCS.Layer.HasValue,  ActiveValue = ActiveValue, InactiveValue = InactiveValue };
            ...
            if (Effect != null) {
                ...
                if (TCS.Blink != 0 && TCS.BlinkIntervalMsNested > 0) {
                    ...
                    Effect = new BlinkEffect() { Name = "Ledwiz {0:00} Column {1:00} Setting {2:00} BlinkEffect Inner".Build(LedWizNr, TCC.Number, SettingNumber), TargetEffectName = Effect.Name, LowValue = TCS.BlinkLow, DurationActiveMs = durationActive, DurationInactiveMs = durationInactive};
                    MakeEffectNameUnique(Effect, Table);
                    Table.Effects.Add(Effect);
                }
                ...
                if (TCS.Blink != 0) {
                    ...
                    Effect = new BlinkEffect() { Name = "Ledwiz {0:00} Column {1:00} Setting {2:00} BlinkEffect".Build(LedWizNr, TCC.Number, SettingNumber), TargetEffectName = Effect.Name, DurationActiveMs = durationActive, DurationInactiveMs = durationInactive };
                ...
                if (TCS.DurationMs > 0 || TCS.Blink > 0) {
                    ...
                    Effect = new DurationEffect() { Name = "Ledwiz {0:00} Column {1:00} Setting {2:00} {3}".Build(LedWizNr, TCC.Number, SettingNumber, N), TargetEffectName = Effect.Name, DurationMs = Duration, RetriggerBehaviour = RetriggerBehaviourEnum.Restart };
                    MakeEffectNameUnique(Effect, Table);
                    Table.Effects.Add(Effect);
                }
                ...
                if (ToyAssignments.ContainsKey(LedWizNr))
                {
                    ...
                    foreach (TableConfigColumn TCC in TC.Columns)
                    {
                        ...
                        if (ToyAssignments[LedWizNr].ContainsKey(TCC.Number))
                        {
                            ...
                            for (int SettingNumber = 0; SettingNumber < TCC.Count; ++SettingNumber)
                            {
                                CreateEffect(TCC, SettingNumber, Table, Toy, LedWizNr, IniFilePath, TC.ShortRomName);
                            }
                        }
                    }
                }
