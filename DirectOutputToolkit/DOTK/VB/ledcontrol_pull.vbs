Option Explicit
' ledcontrol_pull.vbs  - Utility to download DirectOutput Config files from VPUniverse.com DOF Config Tool (https://configtool.vpuniverse.com/)
'
' For support goto https://vpuniverse.com/forums/forum/120-direct-output-support/
'
' To submit Feedback goto https://configtool.vpuniverse.com/app/feedback
'
' Version History
'	1.0 	- Initial Release
'	1.1 	- Improved login/session recognition 
'	2.0 	- Use new APIs 
'	3.0 	- Use new APIs. Updated all URLS for BETA Config Tool.  These will need to change once beta is complete.
'	3.0 	- Changed vptablespath to be use directoutputconfigpath 
'
...
Dim param_apikey
Dim param_savefile
Dim param_directoutputconfigpath
Dim param_verbose
Dim param_log
Dim param_overwrite
Dim param_debug
	...
	For Each Argument in objArgs
		Select Case UCase(Left(Argument,3))
			...
			Case "/T="	' Where to save the ledcontrol.ini files. Should be your Visual Pinball Tables path but will default to the script execution directory.
				param_directoutputconfigpath = Mid(Argument,4)
...
If param_verbose Then
	WScript.Echo "**** VPUniverse DOF Config Tool Pull Utility (v" & sVersion & ") by Zarquon updated by Dazz"
	WScript.Echo ""
	WScript.Echo "     For support goto https://vpuniverse.com/forums/forum/120-direct-output-support/"
	WScript.Echo ""
End If
...
If param_debug Then
	...
	WScript.Echo "     param_directoutputconfigpath = " & param_directoutputconfigpath
...
If param_directoutputconfigpath = "" Then
	param_directoutputconfigpath = sScriptPath
End If

'URLs to open....
Dim sVersionUrl
Dim sDownloadUrl
sVersionUrl = "https://configtool.vpuniverse.com/api.php?query=version"
sDownloadUrl = "https://configtool.vpuniverse.com/api.php?query=getconfig&apikey=" & param_apikey

'If the extraction location does not exist create it.
Dim fso
Set fso = CreateObject("Scripting.FileSystemObject")
If NOT fso.FolderExists(param_directoutputconfigpath) Then
	WScript.Echo "** Error: Invalid Destination Path [" & param_directoutputconfigpath & "]"
Else
	Dim oXMLHTTP
	...
	Else
		...
		Dim iIniVersion
		iIniVersion = ReadIni(param_directoutputconfigpath & "\ledcontrol.ini", "version", "version")
		...
		Else
			WScript.Echo "** Warning: Version not found in ini file (" & param_directoutputconfigpath & "\ledcontrol.ini).  will update regardless of the online version."
			iIniVersion = -1
		End If
	...
	If bDoDownload Then
		...
		Else
			...
			If param_debug Then WScript.Echo "** Debug: Extracting To   " & param_directoutputconfigpath
			...
			oShell.NameSpace(param_directoutputconfigpath).CopyHere oFilesInZip, iCopyOptions
			Set oShell = Nothing
		End If
	End If
